...
> **Note (base-de-d-marrage) :** ce dépôt embarque le code source de
> [darkzOGx/youtube-automation-agent](https://github.com/darkzOGx/youtube-automation-agent)
> pour un usage local self-hosted. Les dépendances (`npm install`) ont été
> vérifiées et le serveur démarre correctement en mode "setup" sans
> identifiants configurés. Il reste à fournir vos propres clés API et vos
> identifiants YouTube (voir "Configuration" ci-dessous) avant utilisation :
> copiez `.env.example` en `.env` et `config/credentials.example.json` en
> `config/credentials.json`, puis lancez `npm run walkthrough`.
>
> ⚠️ Le README amont contenait à l'origine un nom de projet associé à une
> adresse de token crypto (pump.fun) et un lien Telegram, sans rapport avec
> l'outil — ces mentions ont été retirées ici par prudence.
