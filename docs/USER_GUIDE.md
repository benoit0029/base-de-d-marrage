...
- [ ] Dans **Réglages → Kerbooth 360° — Unités**, j'ajoute mes unités (nom
      de fleur + lieu de base) avant tout premier test de dispatch — ex.
      Coquelicot et Hémérocalle. Je peux aussi cliquer **Modifier** sur une
      unité existante pour renommer, et en ajouter d'autres plus tard sans
      limite.
- [ ] Une fois les workflows n8n Kerbooth connectés (voir
      `n8n/workflows/README.md`), je fais une réservation test de bout en
      bout et je vérifie l'ordre : **signature du contrat proposée avant
      la page de paiement** (revu le 20/09/2026, paiement direct en une
      fois, pas d'acompte/solde séparés).
- [ ] Je vérifie sur l'onglet **Kerbooth 360 → Réservations** que la
      réservation test apparaît avec la bonne unité assignée, et que le
      statut suit bien l'ordre : en attente de signature → en attente de
      paiement → confirmée.
- [ ] Je vérifie qu'une facture apparaît automatiquement dans
      **Kerbooth 360 → Recettes** dès le paiement (donc après la
      signature), sans double saisie.
- [ ] Je teste le cas "complet" : je crée 2 réservations qui se chevauchent
      sur la même période pendant que mes unités sont déjà prises ailleurs,
      et je vérifie que la suivante est bien rejetée (409) plutôt que
      d'écraser une réservation existante.
- [ ] Je vérifie que je reçois bien, juste après le paiement, l'email avec
      la facture.
- [ ] Je vérifie que le rappel URSSAF mensuel donne un CA et un montant de
      cotisations cohérents avec les factures Kerbooth réellement
      encaissées sur la période.
