...
- **Google - Kalonia** (type OAuth2 — Google Drive / Google Slides / Google Sheets) : utilisée par `facturation-agence.json` (duplication de template Slides, remplacement de texte, téléchargement PDF, archivage, log Sheet). Un seul compte Google connecté dans n8n couvre les 3 scopes nécessaires (Drive, Slides, Sheets) si les 3 credentials sont créées avec le même compte Google.
- **Hostinger - Kalonia (SMTP)** (type "SMTP", pas OAuth) : envoi de l'email client dans `facturation-agence.json`, depuis l'adresse professionnelle `benoit@kalonia.fr` (boîte mail Hostinger liée au domaine, pas besoin d'un compte Gmail séparé). Identifiants exacts (hôte, port, sécurité) visibles sur Hostinger → Emails → `benoit@kalonia.fr` → Configuration manuelle — généralement `smtp.hostinger.com`, port 465 (SSL) ou 587 (STARTTLS).
