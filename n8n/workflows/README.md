...
- **Outlook - Kalonia (SMTP)** (type "SMTP", pas OAuth) : envoi de l'email client dans `facturation-agence.json`, compatible avec l'adresse `kalonia0029@outlook.fr` existante (pas besoin d'un compte Gmail séparé). Hôte `smtp-mail.outlook.com`, port 587, TLS/STARTTLS activé, identifiants = l'adresse Outlook + son mot de passe (ou un mot de passe d'application si la double authentification est active sur le compte).
