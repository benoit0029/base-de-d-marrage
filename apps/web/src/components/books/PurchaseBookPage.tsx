...
// Livre des achats (Maraîchage) ou registre des achats (Revente, vente de
// marchandises) — lecture seule des Dépenses validées et payées, voir
// lib/livres.
export default async function PurchaseBookPage({ activity, year }: { activity: Activity; year: number }) {
  const currentYear = new Date().getFullYear();
  const [book, closedYears] = await Promise.all([computePurchaseBook(year, activity), listClosedYears()]);
  const locked = closedYears.includes(year);
  const title = activity === "BA_MARAICHAGE" ? "Livre des achats" : "Registre des achats";
      ...
      <div className="rounded-lg border bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <a href={`/api/livres/achats/${year}/pdf?activity=${activity}`} className="font-medium text-slate-700 underline">
              PDF à imprimer
            </a>
          </div>
        </div>
        <p className="mt-2 text-xs text-slate-500">
          Registre obligatoire{activity === "BA_MARAICHAGE" ? "" : " (vente de marchandises)"}, rempli tout seul à
          partir de tes Dépenses validées et payées : rien à ressaisir ici. Classé à la date de paiement, en séparant
          les immobilisations des autres achats, avec la TVA.
        </p>
        {book.unpaidCount > 0 && (
          <p className="mt-2 text-xs text-amber-700">
            ...
            dans le registre à sa date de paiement.
          </p>
        )}
      </div>
