# Documents types — Kerbooth 360° (version corrigée)

Reprend `documents-types-kerbooth360.md` (dossier original) en appliquant les
corrections actées avec Benoît le 20/09/2026 :

1. **RGPD** : la politique de confidentialité (§2) citait encore une
   conservation "6 à 18 mois" — corrigée en suppression immédiate à la
   récupération du matériel, pour rester cohérente avec
   `architecture-technique-kerbooth360.md` et `conformite-rgpd-kerbooth360.md`.
2. **Paiement direct en une fois, plus d'acompte ni de solde séparés**
   (décision du 20/09/2026, revue le même jour) : le Client règle
   l'intégralité du prix à la confirmation du contrat signé, en une seule
   fois — plus de second prélèvement le lendemain de l'événement, donc plus
   de clause d'autorisation de prélèvement différé à prévoir. Simplifie au
   passage la politique d'annulation : un seul montant à traiter, pas un
   acompte "coincé" et un solde à venir.
3. **Politique d'annulation** : le montant réglé **n'est remboursable en
   aucun cas**, quelle que soit la date d'annulation par le Client — aucun
   délai de rétractation ni de courtoisie n'est proposé, cette prestation
   n'y étant de toute façon pas légalement obligée (article 4). Un report
   de date reste possible sous réserve de disponibilité. Répercuté dans les
   CGV (article 5), le contrat, la politique d'annulation autonome (§8) et
   l'email de confirmation.

Tous les champs entre crochets [ ] restent à compléter par Benoît (SIRET
après immatriculation, coordonnées, etc.).

---

## 1. Mentions légales (site web)

*(Inchangé — pas de correction nécessaire ici.)*

```
Le site kerbooth360.fr est édité conjointement par deux micro-entreprises
distinctes exploitant la marque commune "Kerbooth 360°" :

[Prénom NOM 1], entrepreneur individuel (micro-entreprise)
SIRET : [à compléter après immatriculation]

[Prénom NOM 2], entrepreneur individuel (micro-entreprise)
SIRET : [à compléter après immatriculation]

Adresse : [adresse professionnelle]
Email : contact@kerbooth360.fr
Téléphone : [numéro professionnel]

TVA non applicable, article 293 B du Code général des impôts (franchise en base de TVA).

Directeurs de la publication : [Prénom NOM 1] et [Prénom NOM 2]

Hébergement du site : [nom de l'hébergeur, à compléter une fois choisi]

Crédits : conception du site par [toi-même / ton prestataire]
```

⚠️ Ne publie pas le site en ligne tant que les deux SIRET ne sont pas remplis.

---

## 2. Politique de confidentialité (site + événements) — **CORRIGÉE**

```
POLITIQUE DE CONFIDENTIALITÉ — KERBOOTH 360°

Responsable de traitement : [Prénom NOM], [SIRET]

Données collectées :
- Sur le site : nom, email, date d'événement, message (formulaire de contact)
- Lors d'une prestation : la vidéo/photo de chaque invité qui utilise le photobooth, et le contact (email ou téléphone) qu'il choisit pour la recevoir

Finalité :
- Répondre à votre demande de devis
- Vous transmettre votre vidéo/photo lors de l'événement
- Constituer, si vous le souhaitez, une galerie de l'événement à votre attention

Base légale : intérêt légitime de l'organisateur de l'événement pour la galerie privée ; consentement explicite et séparé pour toute utilisation en dehors de cet événement (ex. publication sur les réseaux sociaux de Kerbooth 360°, portfolio).

Durée de conservation : les vidéos/photos d'un événement sont conservées
jusqu'à la récupération du matériel loué, puis supprimées immédiatement,
en votre présence ou celle de l'organisateur de l'événement. Pensez à
récupérer ou télécharger tout contenu que vous souhaitez garder avant ce
moment. Les données de facturation sont conservées 10 ans (obligation
légale comptable).

Vos droits : vous pouvez à tout moment demander l'accès, la rectification ou la suppression de vos données en écrivant à contact@kerbooth360.fr. Vous pouvez également introduire une réclamation auprès de la CNIL (cnil.fr).

Reconnaissance faciale : non activée par défaut. Si une fonctionnalité de ce type venait à être proposée, elle ne serait utilisée qu'avec votre consentement explicite et séparé.

Sous-traitant : nous utilisons le logiciel LumaBooth, édité par Lumasoft
(États-Unis), pour la capture, l'envoi et le stockage temporaire de vos
photos/vidéos. Un contrat de sous-traitance (Data Processing Agreement)
lie Kerbooth 360° à Lumasoft, incluant les clauses contractuelles types
de la Commission européenne pour encadrer ce transfert hors UE.
```

---

## 3. Affichage à côté du photobooth le jour de l'événement (+ texte pour QR code)

*(Déjà cohérent avec la suppression immédiate — inchangé.)*

```
📸 En utilisant ce photobooth, votre vidéo vous est envoyée directement
par le moyen que vous choisissez (email, SMS ou QR code).

Une copie est conservée jusqu'à la récupération du matériel, pour permettre à [nom du client/organisateur]
de récupérer la galerie complète de la soirée.

Vos vidéos ne seront utilisées à des fins de publication ou de promotion
qu'avec votre accord explicite.

Des questions ? contact@kerbooth360.fr — Politique complète : kerbooth360.fr/confidentialite
```

---

## 3bis. Encart d'information précontractuelle — page de l'offre, avant paiement (NOUVEAU)

Obligation légale distincte de l'exemption elle-même (voir article 4 des
CGV ci-dessous) : le client doit être informé de l'absence de droit de
rétractation **avant d'être engagé par le contrat**, pas seulement dans des
CGV qu'il n'a pas forcément lues (art. L221-5 du Code de la consommation).
À afficher sur la page de réservation/devis du site kerbooth360.fr, **avant
le bouton qui lance la signature du contrat** — jamais seulement en bas de
page ou dans un lien CGV séparé.

Texte à afficher (encart visible, pas en petits caractères) :

```
⚠️ Absence de droit de rétractation
Cette prestation est liée à la date de votre événement (activité de
loisirs à date déterminée). Conformément à l'article L221-28 12° du Code
de la consommation, le droit de rétractation ne s'applique donc pas à
cette réservation : une fois le contrat signé et le paiement effectué,
aucun remboursement automatique n'est possible en cas d'annulation
(voir nos CGV, article 5).
```

Exemple d'intégration HTML (encart + case à cocher obligatoire avant de
pouvoir cliquer sur "Signer le contrat", quel que soit le générateur de
site utilisé) :

```html
<div class="kerbooth-legal-notice" style="border:2px solid #b45309;background:#fffbeb;padding:1rem;border-radius:0.5rem;margin:1rem 0;">
  <strong>⚠️ Absence de droit de rétractation</strong>
  <p>
    Cette prestation est liée à la date de votre événement (activité de
    loisirs à date déterminée). Conformément à l'article L221-28 12° du
    Code de la consommation, le droit de rétractation ne s'applique donc
    pas à cette réservation : une fois le contrat signé et le paiement
    effectué, aucun remboursement automatique n'est possible en cas
    d'annulation (voir nos <a href="/cgv">CGV, article 5</a>).
  </p>
  <label>
    <input type="checkbox" name="rétractation_ack" required>
    Je reconnais avoir pris connaissance de l'absence de droit de
    rétractation applicable à cette réservation.
  </label>
</div>
```

La case à cocher n'est pas légalement obligatoire en soi, mais donne une
preuve concrète et horodatée que l'information a bien été présentée avant
l'engagement — utile en cas de litige, bien plus solide qu'une simple
mention statique. Si le site est construit avec un formulaire relié à n8n
(voir `kerbooth-booking-request.json`), cette valeur peut être transmise et
journalisée avec la demande de réservation.

---

## 4. Conditions Générales de Vente (CGV) — **CORRIGÉES**

```
CONDITIONS GÉNÉRALES DE VENTE — KERBOOTH 360°

Article 1 — Objet
Les présentes CGV régissent la location, sans animation ni présence
pendant l'usage, d'un ou plusieurs photobooths 360° par [Prénom NOM],
[SIRET], pour des événements privés ou professionnels.

Article 2 — Réservation et paiement
La réservation est proposée à la signature électronique (via Yousign) du
devis et du contrat de location. Une fois le contrat signé, elle n'est
définitivement confirmée qu'à réception du paiement intégral du prix
(100 € pour la formule Essentiel, 200 € pour la formule Populaire, montant
sur devis pour la formule Entreprise), réglé en une seule fois, en ligne.

Article 3 — Tarifs
Les tarifs sont ceux indiqués sur le devis accepté par le client.
[Montant] TVA non applicable, art. 293 B du CGI.

Article 4 — Absence de droit de rétractation
Conformément à l'article L221-28 12° du Code de la consommation, le droit
de rétractation ne s'applique pas aux prestations d'activités de loisirs
devant être fournies à une date déterminée. La présente location, liée
à la date de votre événement, n'ouvre donc pas droit à rétractation après
acceptation du devis et paiement du prix.

Article 5 — Annulation
Le prix versé à la réservation n'est remboursable en aucun cas, quelle que
soit la date d'annulation par le Client — aucun délai de rétractation ni
de courtoisie n'est accordé (voir article 4). Un report de date reste
possible sous réserve de disponibilité du matériel, à convenir avec le
Prestataire. En cas d'annulation à l'initiative de Kerbooth 360° (panne
matérielle, indisponibilité imprévue), l'intégralité des sommes versées
par le Client est remboursée, sans autre indemnité.

Article 5bis — Dépôt de garantie et état des lieux
Une caution de 1 500 € par chèque est demandée à la livraison,
restituée après récupération du matériel en bon état. Un état des
lieux contradictoire (modèle en section 5bis ci-dessous) est établi et
signé par les deux parties à la livraison et à la restitution.

Article 6 — Déroulement de la location
Livraison et installation avant le début du créneau loué (journée ou
weekend selon la formule), récupération à la fin de celui-ci. **Le
Client n'est pas assisté par un animateur pendant la durée de la
location** : il assure la garde du matériel et en répond conformément
à l'article 1732 du Code civil (responsabilité du locataire pour les
dégradations ou pertes survenues pendant la durée de la location, sauf
à prouver qu'elles ont eu lieu sans sa faute). Un espace dédié, une
prise électrique à proximité et un accès pour le transport du matériel
sont à prévoir par le Client.

Article 7 — Responsabilité et assurance
Kerbooth 360° est couvert par une assurance responsabilité civile
professionnelle et une assurance multirisque pour son matériel. **Le
Client doit disposer de sa propre assurance couvrant sa responsabilité
en tant que locataire** (matériel confié + usage par ses invités)
pendant toute la durée de la location.

Article 8 — Droit à l'image et RGPD
Voir la Politique de confidentialité, remise au client avant l'événement.

Article 9 — Droit applicable et litiges
Les présentes CGV sont soumises au droit français. En cas de litige, une
solution amiable sera recherchée avant toute action judiciaire.
```

**Rappel légal (déjà signalé dans le dossier original, toujours valable)** :
l'absence de droit de rétractation (article 4) doit être mentionnée de
façon visible **dès la page de présentation de l'offre**, avant validation
de la commande — pas seulement dans les CGV.

---

## 5. Contrat de location type — **CORRIGÉ** (à signer électroniquement via Yousign)

```
CONTRAT DE LOCATION — PHOTOBOOTH 360°

Entre :
[Prénom NOM], entrepreneur individuel, SIRET [·], ci-après "le Loueur"

Et :
[Nom du client], ci-après "le Locataire"

Il a été convenu ce qui suit :

1. Objet : mise à disposition, sans animation ni présence pendant
   l'usage, d'un photobooth 360° pour l'événement du [date], à [lieu].

2. Formule retenue : [Essentiel (1 jour) / Populaire (weekend) /
   Entreprise, sur devis]

3. Montant total : [montant] €, réglé en une seule fois à la confirmation
   du présent contrat signé, non remboursable en cas d'annulation (voir
   CGV article 5).

4. Livraison et récupération : le Loueur installe le matériel avant le
   début du créneau loué et le récupère à la fin. Le Locataire n'est
   pas assisté par un animateur pendant la durée de la location.

5. Garde du matériel : à compter de la livraison et jusqu'à la
   récupération, le Locataire a la garde du matériel et en répond
   conformément à l'article 1732 du Code civil.

6. Dépôt de garantie : un chèque de caution de 1 500 € est remis au
   Loueur à la livraison (cf. état des lieux d'entrée ci-joint) et
   restitué à la récupération, sauf dégradation constatée à l'état des
   lieux de sortie.

7. Obligations du Locataire : mise à disposition d'un espace adapté
   (surface, accès électrique), information de ses invités sur
   l'utilisation de leurs images (cf. Politique de confidentialité
   annexée), souscription d'une assurance couvrant sa responsabilité
   de locataire.

8. CGV : le Locataire reconnaît avoir pris connaissance des CGV de
   Kerbooth 360°, annexées au présent contrat, et notamment de
   l'absence de droit de rétractation applicable (art. L221-28 12° du
   Code de la consommation) et du caractère non remboursable du prix
   versé (article 5 des CGV).

Signé électroniquement via Yousign par les deux parties.

Le Loueur                               Le Locataire
[Signature électronique]                [Signature électronique]
```

---

## 5bis. État des lieux du matériel

*(Inchangé.)*

```
ÉTAT DES LIEUX — PHOTOBOOTH 360° N°[·]

Événement du [date], contrat de location N°[·]

☐ À LA LIVRAISON          ☐ À LA RÉCUPÉRATION   (cocher la bonne case)

Date et heure : [·]

État constaté :
- Plateforme rotative : ☐ Bon état ☐ Dégât constaté : [description]
- Bras/structure : ☐ Bon état ☐ Dégât constaté : [description]
- Smartphone associé : ☐ Bon état ☐ Dégât constaté : [description]
- Éclairage / anneau lumineux : ☐ Bon état ☐ Dégât constaté : [description]
- Accessoires (câbles, housse, props) : ☐ Complets ☐ Manquants : [liste]
- Photos jointes : ☐ Oui (recommandé, horodatées)

Chèque de caution (1 500 €) :
☐ Remis à la livraison — n° de chèque : [·], banque : [·]
☐ Restitué à la récupération (déchiré/rendu en main propre)
☐ Encaissé (dégât constaté) — montant retenu : [·] €, motif : [·]

Signatures des deux parties :

Le Loueur                               Le Locataire
[Signature]                             [Signature]
```

---

## 6. Devis type (formule Entreprise uniquement)

*(Obsolète pour Essentiel/Populaire, prix fixe direct à la facture.)*

```
                              KERBOOTH 360°
                    [Prénom NOM de l'associé émetteur]
                         SIRET : [·]

DEVIS N° [·]                                    Date : [·]
Valable jusqu'au : [·]

[Adresse]

Devis pour : [Nom du client]
Événement : [type] du [date] à [lieu]

Désignation                                              Montant
--------------------------------------------------------------
Location photobooth 360° — formule [·]                    [·] €
Déplacement (si hors zone gratuite)                       [·] €
--------------------------------------------------------------
Total                                                      [·] €
TVA non applicable, art. 293 B du CGI

Paiement intégral à la confirmation (obligatoire, en ligne, non
remboursable en cas d'annulation) : 100 € (formule Essentiel) ou 200 €
(formule Populaire), réglés en une seule fois une fois le contrat signé.
Formule Entreprise : contact direct, paiement à réception de facture,
échéance 30 jours (hors flux en ligne).

Conditions : voir CGV jointes. Ce devis vaut contrat une fois signé
électroniquement via Yousign avec le contrat de location joint, puis
réglé intégralement.
```

---

## 7. Facture type

*(Réglée intégralement au paiement en ligne — plus de "reste dû" à
suivre, contrairement à l'ancien découpage acompte/solde.)*

```
                              KERBOOTH 360°
                    [Prénom NOM de l'associé émetteur]
                         SIRET : [·]

FACTURE N° [·]                                  Date : [·]

[Adresse]

Facturé à : [Nom du client]
[Adresse du client, si professionnel]

Désignation                                              Montant
--------------------------------------------------------------
Location photobooth 360° — [événement du ·]               [·] €
--------------------------------------------------------------
Total TTC                                                  [·] €
TVA non applicable, art. 293 B du CGI

Réglé le [date]                                            [·] €

Mode de paiement : [carte en ligne / virement, formule Entreprise]
```

**Principe retenu** : "Kerbooth 360°" reste l'en-tête visible de chaque
facture, mais le nom civil et le SIRET de la personne qui a réellement
réalisé l'événement apparaissent juste en dessous — voir
`business-photobooth-360.md` et la note d'architecture sur la facturation
(chaque associé facture, encaisse et déclare ce qu'il a lui-même réalisé).

---

## 8. Politique d'annulation (document autonome) — **CORRIGÉE**

```
POLITIQUE D'ANNULATION — KERBOOTH 360°

- Le prix versé à la réservation n'est remboursable en aucun cas, quelle
  que soit la date d'annulation. Cette prestation est exclue du droit de
  rétractation légal (art. L221-28 12° du Code de la consommation,
  activités de loisirs à date déterminée) et aucun délai de courtoisie
  n'est accordé.
- Report de date : possible sous réserve de disponibilité du matériel,
  à demander par écrit dès que possible.
- Annulation à l'initiative de Kerbooth 360° (panne matérielle,
  indisponibilité imprévue) : remboursement intégral de toutes les
  sommes versées, sans autre indemnité.

Cette politique est reprise dans les CGV (article 5) et rappelée sur
chaque devis/contrat de location.
```

---

## 9. Email de confirmation de réservation — **CORRIGÉ**

```
Objet : Votre réservation Kerbooth 360° est confirmée — [date de l'événement]

Bonjour [Prénom],

Votre réservation est confirmée pour le [date], formule [Essentiel/Populaire].
Voici un récapitulatif :

- Montant réglé : [100€/200€], payé intégralement en une fois (non
  remboursable en cas d'annulation, voir notre politique d'annulation)
- Contrat de location et CGV : signés électroniquement, vous trouverez
  une copie en pièce jointe
- Votre facture est jointe à cet email

⚠️ IMPORTANT — dépôt de garantie :
Merci de prévoir un chèque de caution de 1 500 €, à remettre en main
propre à la livraison. Sans ce chèque, nous ne pourrons pas installer
le matériel le jour J. Ce chèque vous sera intégralement restitué à la
récupération, sauf dégradation constatée à l'état des lieux de sortie.
Il ne sera pas encaissé sauf en cas de dommage.

Des questions avant votre événement ? Répondez simplement à cet email.

À bientôt,
Kerbooth 360°
```

---

## Ce qu'il reste à compléter (inchangé du dossier original)

- Le SIRET, une fois obtenu (ajout d'activité, cf. feuille de route).
- L'adresse d'hébergement du site, une fois choisie.
- **⚠️ À intégrer sur le site kerbooth360.fr (hors de ce dépôt) — point
  confirmé le 20/09/2026** : l'encart d'information précontractuelle sur
  l'absence de droit de rétractation (texte + snippet HTML prêts à
  l'emploi, voir §3bis ci-dessus) doit être affiché sur la page de
  réservation, **avant** que le client ne signe le contrat et ne paie —
  la mentionner seulement dans les CGV/contrat déjà signés ne suffit pas
  à couvrir cette obligation d'information précontractuelle (art. L221-5
  et L221-28 12° du Code de la consommation).
- ⚠️ Ces modèles corrigés couvrent les mentions identifiées comme
  nécessaires ; une relecture par un avocat reste recommandée avant
  publication définitive.
