...
- ✅ **SPF/DKIM/DMARC** déjà configurés automatiquement par Hostinger sur `kalonia.fr` (vérifié dans la zone DNS Cloudflare) — rien à faire. DMARC est en `p=none` (surveillance uniquement) — **à durcir plus tard en `p=quarantine`** une fois confiant que tout fonctionne bien.
- DNS de `kalonia.fr` géré sur **Cloudflare** (nameservers délégués), pas sur Hostinger — décision assumée (sécurité/fiabilité Cloudflare supérieure, éviter le risque de migration). Le VPS (A record) est en "DNS only" (non proxié), correct pour un serveur SSH/n8n.
- **Nettoyage mineur à faire un jour** (sans urgence, aucun risque à les laisser) : 2 enregistrements DNS orphelins de l'exploration Lovable abandonnée (`notify.kalonia.fr` NS vers lovable.cloud, `_lovable-email.kalonia.fr` TXT) — à supprimer sur Cloudflare quand on y pense.
- ✅ **Monitoring d'erreurs** : Sentry intégré (`@sentry/react`, init dans `src/main.tsx`, inactif tant que `VITE_SENTRY_DSN` n'est pas défini). Compte créé avec stockage des données en **UE** (région Allemagne, `ingest.de.sentry.io`) — règle le point RGPD. Seul "Error monitoring" activé (pas de Session Replay, qui capturerait l'écran donc potentiellement des données clients affichées). `sendDefaultPii: false` pour limiter la collecte de données personnelles. Variable `VITE_SENTRY_DSN` ajoutée sur Vercel (type "Config", pas "Secret" — Vercel refuse "Secret" pour les préfixes `VITE_*` puisqu'ils finissent de toute façon exposés côté client).
- ✅ **Registre RGPD des sous-traitants** créé (`RGPD-sous-traitants.md` à la racine du dépôt) — liste tous les outils actifs et en pause, données concernées, hébergement, à formaliser (DPA) avant onboarding de vrais clients.
